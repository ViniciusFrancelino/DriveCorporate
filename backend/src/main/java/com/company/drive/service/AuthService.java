...
@Service public class AuthService {
 ...
 public LoginResponse login(LoginRequest r){String email=r.getEmail().trim().toLowerCase();auth.authenticate(new UsernamePasswordAuthenticationToken(email,r.getPassword()));User u=users.findByEmail(email).orElseThrow();return new LoginResponse(jwt.generateToken(u),UserResponse.from(u));}
}
